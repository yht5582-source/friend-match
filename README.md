...
`POST /api/seed` 灌進來的 12 位會員，信箱是 `demo.*@example.com`——**不存在的位址**。
它們會正常出現在配對結果、參與計分，但被標示為「示範帳號」且**不能被聯絡**：
`/api/compose` 與 `/api/contact` 都會擋下，畫面上按鈕也是停用的。

否則真實使用者會對著一個永遠沒人回覆的信箱寫信，還白白用掉當天的取用額度。

判斷方式是 `isDemo` 旗標**或**信箱結尾是 `@example.com`，所以先前灌進去、
沒有旗標的舊資料也會被正確辨識，不需要搬移資料。
